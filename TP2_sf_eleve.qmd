...
```{r}
communes_metro_bret <- communes_metro %>% 
  filter(reg == "53") %>% 
  select(code, libelle, epc, dep, surf)
#Non elle garde la variable Geom qui permet de tracer les contours des communes
```
...
```{r}
communes_metro_bret$surf2 <- st_area(communes_metro_bret$geom)
```

10. Modifier la variable créée pour la convertir en km².

```{r}
communes_metro_bret <- communes_metro_bret %>% 
...
```{r}
dept_bretagne <- communes_metro_bret %>% 
  group_by(dep) %>% 
  summarise(surf_dep = sum(surf))
...
```{r}
new_dept_bretagne <- communes_metro_bret %>% 
  group_by(dep) %>% 
  summarise(geom2 = st_union(geom))

plot(new_dept_bretagne)
```
...
```{r}
centroid_bret <- st_centroid(dept_bretagne)
class(centroid_bret$geom)  #Ce sont des données ponctuelles
```
...
plot(st_geometry(centroid_bret), add= TRUE)
```
...
centroid_bret <- centroid_bret %>% 
  left_join(dep_lib, by="dep")
...
```{r}
centroid_coords <- st_coordinates(centroid_bret)
str(centroid_coords)
#On obteient un dataframe avec deux colonnes uniquement : les colonnes X et Y

centroid_coords <- centroid_coords %>% 
  bind_cols(centroid_bret %>% 
              select(dep,dep_lib) %>% 
              st_drop_geometry()
            )
```
...
```{r}
commune_centroid_bret <- st_intersects(centroid_bret,communes_metro_bret)
#Il s'agit d'une liste égale au nombre d'éléments du premier paramètre entré

communes_metro_bret %>% slice(centroid_bret) %>% select(libelle)
```
...
```{r}
st_intersection(communes_metro_bret,centroid_bret)
#Fonction dédiée à l'intersection des 2 géométries associées aux 2 objets sf

#Avec la focntion st_within(), on regarde si le premier paramètre est contenu dans le second
st_within(centroid_bret, communes_metro_bret)
```
...
```{r}
st_distance((centroid_bret,communes_metro_bret %>% 
               filter(libelle %in% c("Saint-Brieux=","Quimper","Rennes","Vannes"))) %>% data.frame()   #tibble() renverra une seule colonne
